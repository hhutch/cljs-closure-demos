...
goog.ui.ComboBox.prototype.enterDocument = function() {
  ...
  a.listen(this.inputHandler_, goog.events.InputHandler.EventType.INPUT, this.onInputChange_);
...
goog.ui.ComboBox.prototype.setValue = function(a) {
  this.logger_.info("setValue() - " + a);
  this.labelInput_.getValue() != a && (this.labelInput_.setValue(a), this.dispatchEvent(goog.ui.Component.EventType.CHANGE))
};
goog.ui.ComboBox.prototype.getValue = function() {
  return this.labelInput_.getValue()
};
goog.ui.ComboBox.prototype.getToken = function() {
  return goog.string.htmlEscape(goog.string.trim(this.labelInput_.getValue().toLowerCase()))
};
goog.ui.ComboBox.prototype.setupMenu_ = function() {
  var a = this.menu_;
  a.setVisible(false);
  a.setAllowAutoFocus(false);
  a.setAllowHighlightDisabled(true)
};
goog.ui.ComboBox.prototype.maybeShowMenu_ = function(a) {
  var b = this.menu_.isVisible(), c = this.getNumberOfVisibleItems_();
  b && c == 0 ? (this.logger_.fine("no matching items, hiding"), this.hideMenu_()) : !b && c > 0 && (a && (this.logger_.fine("showing menu"), this.setItemVisibilityFromToken_(""), this.setItemHighlightFromToken_(this.getToken())), goog.Timer.callOnce(this.clearDismissTimer_, 1, this), a = goog.style.getPageOffset(this.getElement()), this.menu_.setPosition(a.x, a.y + this.getElement().offsetHeight), this.showMenu_())
};
...
goog.ui.ComboBox.prototype.clearDismissTimer_ = function() {
  if(this.dismissTimer_) {
    goog.Timer.clear(this.dismissTimer_), this.dismissTimer_ = null
  }
};
goog.ui.ComboBox.prototype.onComboMouseDown_ = function(a) {
  if(this.enabled_ && (a.target == this.getElement() || a.target == this.input_ || goog.dom.contains(this.button_, a.target))) {
    this.menu_.isVisible() ? (this.logger_.fine("Menu is visible, dismissing"), this.dismiss()) : (this.logger_.fine("Opening dropdown"), this.maybeShowMenu_(true), goog.userAgent.OPERA && this.input_.focus(), this.input_.select(), this.menu_.setMouseButtonPressed(true), a.preventDefault())
  }
  a.stopPropagation()
};
goog.ui.ComboBox.prototype.onDocClicked_ = function(a) {
  goog.dom.contains(this.menu_.getElement(), a.target) || (this.logger_.info("onDocClicked_() - dismissing immediately"), this.dismiss())
};
goog.ui.ComboBox.prototype.onMenuSelected_ = function(a) {
  this.logger_.info("onMenuSelected_()");
  if(this.dispatchEvent(new goog.ui.ItemEvent(goog.ui.Component.EventType.ACTION, this, a.target))) {
    var b = a.target.getValue();
    this.logger_.fine("Menu selection: " + b + ". Dismissing menu");
    this.setValue(goog.string.unescapeEntities(b));
    this.dismiss()
  }
  a.stopPropagation()
};
goog.ui.ComboBox.prototype.onInputBlur_ = function() {
...
goog.ui.ComboBox.prototype.handleKeyEvent = function(a) {
  ...
  if(b && this.menu_.handleKeyEvent(a)) {
    return true
  }
  var c = false;
  switch(a.keyCode) {
    case goog.events.KeyCodes.ESC:
      b && (this.logger_.fine("Dismiss on Esc: " + this.labelInput_.getValue()), this.dismiss(), c = true);
      break;
    case goog.events.KeyCodes.TAB:
      if(b && (b = this.menu_.getHighlighted())) {
        this.logger_.fine("Select on Tab: " + this.labelInput_.getValue()), b.performActionInternal(a), c = true
      }
      break;
    case goog.events.KeyCodes.UP:
    ;
    case goog.events.KeyCodes.DOWN:
      b || (this.logger_.fine("Up/Down - maybe show menu"), this.maybeShowMenu_(true), c = true)
  }
  c && a.preventDefault();
  return c
};
goog.ui.ComboBox.prototype.onInputChange_ = function() {
...
goog.ui.ComboBox.prototype.setItemVisibilityFromToken_ = function(a) {
  this.logger_.info("setItemVisibilityFromToken_() - " + a);
  for(var b = false, c = 0, d = !this.matchFunction_(a, this.lastToken_), e = 0, f = this.menu_.getChildCount();e < f;e++) {
    var g = this.menu_.getChildAt(e);
    if(g instanceof goog.ui.MenuSeparator) {
      g.setVisible(b), b = false
    }else {
      if(g instanceof goog.ui.MenuItem) {
        if(!g.isVisible() && !d) {
          continue
        }
        var h = g.getCaption(), h = this.isItemSticky_(g) || h && this.matchFunction_(h.toLowerCase(), a);
        typeof g.setFormatFromToken == "function" && g.setFormatFromToken(a);
        g.setVisible(!!h);
        b = h || b
      }else {
        b = g.isVisible() || b
...
goog.ui.ComboBox.prototype.setItemHighlightFromToken_ = function(a) {
  this.logger_.info("setItemHighlightFromToken_() - " + a);
  if(a != "") {
    for(var b = 0, c = this.menu_.getChildCount();b < c;b++) {
...
goog.ui.ComboBox.prototype.isItemSticky_ = function(a) {
  return typeof a.isSticky == "function" && a.isSticky()
};
goog.ui.ComboBoxItem = function(a, b, c) {
...
goog.ui.ComboBoxItem.prototype.setFormatFromToken = function(a) {
  if(this.isEnabled()) {
    var a = goog.string.regExpEscape(a), b = this.getCaption();
    if(b) {
      this.getElement().innerHTML = b.replace(RegExp(a, "i"), function(a) {
        return"<b>" + a + "</b>"
...
var combobox = {setup:function() {
  var a = new goog.debug.DivConsole(clojure.browser.dom.get_element.call(null, "log")), b = clojure.browser.dom.get_element.call(null, "combo1"), c = new goog.ui.ComboBox, d = new goog.ui.ComboBoxItem("New Folder..."), e = clojure.browser.dom.get_element.call(null, "combo2"), f = new goog.ui.ComboBox, g = new goog.ui.ComboBoxItem("Select a Color...");
  goog.debug.LogManager.getRoot().setLevel(goog.debug.Logger.Level.ALL);
  a.setCapturing(true);
  d.setSticky(true);
  c.setUseDropdownArrow(true);
  c.setDefaultText("Select a folder...");
  c.addItem(new goog.ui.ComboBoxItem("Inbox"));
  c.addItem(new goog.ui.ComboBoxItem("Bills & statements"));
  c.addItem(new goog.ui.ComboBoxItem("Cal alumni"));
  c.addItem(new goog.ui.ComboBoxItem("Calendar Stuff"));
  c.addItem(new goog.ui.ComboBoxItem("Design"));
  c.addItem(new goog.ui.ComboBoxItem("Music"));
  c.addItem(new goog.ui.ComboBoxItem("Netflix"));
  c.addItem(new goog.ui.ComboBoxItem("Personal"));
  c.addItem(new goog.ui.ComboBoxItem("Photos"));
  c.addItem(new goog.ui.ComboBoxItem("Programming"));
  c.addItem(d);
  c.render(b);
  g.setSticky(true);
  g.setEnabled(true);
  f.setDefaultText("Select a color...");
  f.addItem(new goog.ui.ComboBoxItem("Red"));
  f.addItem(new goog.ui.ComboBoxItem("Maroon"));
  f.addItem(new goog.ui.ComboBoxItem("Green"));
  f.addItem(new goog.ui.ComboBoxItem("Blue"));
  f.addItem(new goog.ui.ComboBoxItem("Royal Blue"));
  f.addItem(new goog.ui.ComboBoxItem("Yellow"));
  f.addItem(new goog.ui.ComboBoxItem("Magenta"));
  f.addItem(new goog.ui.ComboBoxItem("Mouve"));
  f.addItem(new goog.ui.ComboBoxItem("Grey"));
  f.addItemAt(g, 0);
  f.render(e);
  goog.events.listen(c, "change", function(a) {
    return goog.dom.setTextContent(clojure.browser.dom.get_element.call(null, "v"), a.target.getValue())
  });
  goog.events.listen(f, "change", function(a) {
    return goog.dom.setTextContent(clojure.browser.dom.get_element.call(null, "v"), a.target.getValue())
  });
  return goog.events.listen(clojure.browser.dom.get_element.call(null, "clearlog"), goog.events.EventType.CLICK, function() {
    return a.clear()
  })
}};
goog.exportSymbol("combobox.setup", combobox.setup);
