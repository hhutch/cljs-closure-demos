...
goog.ui = {};
goog.ui.AutoComplete = function(a, b, c) {
  goog.events.EventTarget.call(this);
  this.matcher_ = a;
  this.selectionHandler_ = c;
  this.renderer_ = b;
  goog.events.listen(b, [goog.ui.AutoComplete.EventType.HILITE, goog.ui.AutoComplete.EventType.SELECT, goog.ui.AutoComplete.EventType.CANCEL_DISMISS, goog.ui.AutoComplete.EventType.DISMISS], this);
  this.token_ = null;
  this.rows_ = [];
  this.hiliteId_ = -1;
  this.firstRowId_ = 0;
...
goog.ui.AutoComplete.EventType = {HILITE:"hilite", SELECT:"select", DISMISS:"dismiss", CANCEL_DISMISS:"canceldismiss", UPDATE:"update", SUGGESTIONS_UPDATE:"suggestionsupdate"};
goog.ui.AutoComplete.prototype.getRenderer = function() {
  return this.renderer_
};
goog.ui.AutoComplete.prototype.handleEvent = function(a) {
...
goog.ui.AutoComplete.prototype.setMaxMatches = function(a) {
  this.maxMatches_ = a
};
goog.ui.AutoComplete.prototype.setAutoHilite = function(a) {
  this.autoHilite_ = a
};
goog.ui.AutoComplete.prototype.setAllowFreeSelect = function(a) {
  this.allowFreeSelect_ = a
};
goog.ui.AutoComplete.prototype.setWrap = function(a) {
  this.wrap_ = a
};
goog.ui.AutoComplete.prototype.setTriggerSuggestionsOnUpdate = function(a) {
  this.triggerSuggestionsOnUpdate_ = a
};
goog.ui.AutoComplete.prototype.setToken = function(a, b) {
  if(this.token_ != a) {
    this.token_ = a, this.matcher_.requestMatchingRows(this.token_, this.maxMatches_, goog.bind(this.matchListener_, this), b), this.cancelDelayedDismiss()
  }
};
goog.ui.AutoComplete.prototype.getTarget = function() {
  return this.target_
};
goog.ui.AutoComplete.prototype.setTarget = function(a) {
  this.target_ = a
};
...
goog.ui.AutoComplete.prototype.hiliteNext = function() {
  var a = this.firstRowId_ + this.rows_.length - 1;
  if(this.hiliteId_ >= this.firstRowId_ && this.hiliteId_ < a) {
    return this.hiliteId(this.hiliteId_ + 1), true
  }else {
    if(this.hiliteId_ == -1) {
      return this.hiliteId(this.firstRowId_), true
    }else {
      if(this.hiliteId_ == a) {
  ...
  if(this.hiliteId_ > this.firstRowId_) {
    ...
    if(this.allowFreeSelect_ && this.hiliteId_ == this.firstRowId_) {
      ...
      if(this.wrap_ && (this.hiliteId_ == -1 || this.hiliteId_ == this.firstRowId_)) {
        return this.hiliteId(this.firstRowId_ + this.rows_.length - 1), true
      }
    }
  }
  return false
};
goog.ui.AutoComplete.prototype.hiliteId = function(a) {
  this.hiliteId_ = a;
  this.renderer_.hiliteId(a);
  return this.getIndexOfId(a) != -1
};
goog.ui.AutoComplete.prototype.selectHilited = function() {
  ...
  if(a != -1) {
    ...
    this.dismiss();
    b || (this.dispatchEvent({type:goog.ui.AutoComplete.EventType.UPDATE, row:a}), this.triggerSuggestionsOnUpdate_ && this.selectionHandler_.update(true));
    return true
  }else {
    return this.dismiss(), this.dispatchEvent({type:goog.ui.AutoComplete.EventType.UPDATE, row:null}), false
  }
};
...
goog.ui.AutoComplete.prototype.cancelDelayedDismiss = function() {
  window.setTimeout(goog.bind(function() {
    if(this.dismissTimer_) {
      window.clearTimeout(this.dismissTimer_), this.dismissTimer_ = null
    }
  }, this), 10)
};
...
goog.ui.AutoComplete.prototype.matchListener_ = function(a, b, c) {
  this.token_ == a && this.renderRows(b, c)
};
goog.ui.AutoComplete.prototype.renderRows = function(a, b) {
  var c = b ? this.getIndexOfId(this.hiliteId_) : null;
  this.firstRowId_ += this.rows_.length;
  this.rows_ = a;
  ...
  this.autoHilite_ && d.length != 0 && this.token_ ? this.hiliteId(c != null ? this.getIdOfIndex_(c) : this.firstRowId_) : this.hiliteId_ = -1;
  this.dispatchEvent(goog.ui.AutoComplete.EventType.SUGGESTIONS_UPDATE)
};
goog.ui.AutoComplete.prototype.getIndexOfId = function(a) {
  a -= this.firstRowId_;
  return a < 0 || a >= this.rows_.length ? -1 : a
};
goog.ui.AutoComplete.prototype.getIdOfIndex_ = function(a) {
  return this.firstRowId_ + a
};
goog.ui.AutoComplete.prototype.attachInputs = function(a) {
  var b = this.selectionHandler_;
  b.attachInputs.apply(b, arguments)
};
goog.ui.AutoComplete.prototype.detachInputs = function(a) {
  var b = this.selectionHandler_;
...
goog.ui.AutoComplete.InputHandler = function(a, b, c, d) {
  goog.Disposable.call(this);
  d = d || 150;
  this.setSeparators(goog.isDefAndNotNull(a) ? a : goog.ui.AutoComplete.InputHandler.STANDARD_LIST_SEPARATORS);
  this.literals_ = b || "";
  this.preventDefaultOnTab_ = this.multi_ = c != null ? c : true;
  this.timer_ = d > 0 ? new goog.Timer(d) : null;
...
goog.ui.AutoComplete.InputHandler.prototype.attachAutoComplete = function(a) {
  this.ac_ = a
};
...
goog.ui.AutoComplete.InputHandler.prototype.setValue = function(a) {
...
goog.ui.AutoComplete.InputHandler.prototype.setCursorPosition = function(a) {
  goog.dom.selection.setStart(this.activeElement_, a);
  goog.dom.selection.setEnd(this.activeElement_, a)
};
goog.ui.AutoComplete.InputHandler.prototype.attachInput = function(a) {
  goog.dom.a11y.setState(a, "haspopup", true);
  ...
  this.activeElement_ || this.activateHandler_.listen(a, goog.events.EventType.KEYDOWN, this.onKeyDownOnInactiveElement_)
};
goog.ui.AutoComplete.InputHandler.prototype.detachInput = function(a) {
...
goog.ui.AutoComplete.InputHandler.prototype.attachInputs = function(a) {
  for(var b = 0;b < arguments.length;b++) {
    this.attachInput(arguments[b])
  }
};
goog.ui.AutoComplete.InputHandler.prototype.detachInputs = function(a) {
  for(var b = 0;b < arguments.length;b++) {
    this.detachInput(arguments[b])
  }
};
goog.ui.AutoComplete.InputHandler.prototype.selectRow = function(a, b) {
  this.setTokenText(a.toString(), b);
  return false
};
goog.ui.AutoComplete.InputHandler.prototype.setTokenText = function(a, b) {
  if(goog.isDef(b) ? b : this.multi_) {
    ...
    this.whitespaceWrapEntries_ && (c != 0 && !goog.string.isEmpty(d[c - 1]) && (e = " " + e), c == d.length - 1 && (e += " "));
    if(e != d[c]) {
      d[c] = e;
      e = this.activeElement_;
      goog.userAgent.GECKO && e.blur();
      e.value = d.join("");
...
goog.ui.AutoComplete.InputHandler.prototype.setSeparators = function(a) {
  this.separators_ = a;
...
goog.ui.AutoComplete.InputHandler.prototype.setUpsideDown = function(a) {
  this.upsideDown_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setWhitespaceWrapEntries = function(a) {
  this.whitespaceWrapEntries_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setGenerateNewTokenOnLiteral = function(a) {
  this.generateNewTokenOnLiteral_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setTrimmingRegExp = function(a) {
  this.trimmer_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setPreventDefaultOnTab = function(a) {
  this.preventDefaultOnTab_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setSeparatorCompletes = function(a) {
  this.separatorSelects_ = this.separatorUpdates_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setSeparatorSelects = function(a) {
...
goog.ui.AutoComplete.InputHandler.prototype.setRowJustSelected = function(a) {
  this.rowJustSelected_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.setThrottleTime = function(a) {
  a < 0 ? (this.timer_.dispose(), this.timer_ = null) : this.timer_ ? this.timer_.setInterval(a) : this.timer_ = new goog.Timer(a)
};
goog.ui.AutoComplete.InputHandler.prototype.getUpdateDuringTyping = function() {
  return this.updateDuringTyping_
};
goog.ui.AutoComplete.InputHandler.prototype.setUpdateDuringTyping = function(a) {
  this.updateDuringTyping_ = a
};
goog.ui.AutoComplete.InputHandler.prototype.handleKeyEvent = function(a) {
  switch(a.keyCode) {
    case goog.events.KeyCodes.DOWN:
      if(this.ac_.isOpen()) {
        return this.moveDown_(), a.preventDefault(), true
      }else {
        if(!this.multi_) {
          return this.update(true), a.preventDefault(), true
        }
      }
      break;
    case goog.events.KeyCodes.UP:
      if(this.ac_.isOpen()) {
        return this.moveUp_(), a.preventDefault(), true
      }
      break;
    case goog.events.KeyCodes.TAB:
      if(this.ac_.isOpen() && !a.shiftKey) {
        if(this.update(), this.ac_.selectHilited() && this.preventDefaultOnTab_) {
    ...
    case goog.events.KeyCodes.ENTER:
      if(this.ac_.isOpen()) {
        if(this.update(), this.ac_.selectHilited()) {
          return a.preventDefault(), a.stopPropagation(), true
        }
      }else {
        this.ac_.dismiss()
      }
    ...
    case goog.events.KeyCodes.ESC:
      if(this.ac_.isOpen()) {
        return this.ac_.dismiss(), a.preventDefault(), a.stopPropagation(), true
      }
      break;
    case goog.events.KeyCodes.WIN_IME:
      if(!this.waitingForIme_) {
        return this.startWaitingForIme_(), true
      }
      break;
    default:
...
goog.ui.AutoComplete.InputHandler.prototype.handleSeparator_ = function(a) {
  var b = this.multi_ && a.charCode && this.separators_.indexOf(String.fromCharCode(a.charCode)) != -1;
  this.separatorUpdates_ && b && this.update();
  return this.separatorSelects_ && b && this.ac_.selectHilited() ? (a.preventDefault(), true) : false
};
...
goog.ui.AutoComplete.InputHandler.prototype.handleFocus = function(a) {
  this.activateHandler_.removeAll();
  this.ac_ && this.ac_.cancelDelayedDismiss();
  if(a.target != this.activeElement_) {
    this.activeElement_ = a.target || null, this.timer_ && (this.timer_.start(), this.eh_.listen(this.timer_, goog.Timer.TICK, this.onTick_)), this.lastValue_ = this.getValue(), this.addKeyEvents_()
  }
};
goog.ui.AutoComplete.InputHandler.prototype.handleBlur = function() {
  if(this.activeElement_) {
    this.removeKeyEvents_(), this.activeElement_ = null, this.timer_ && (this.timer_.stop(), this.eh_.unlisten(this.timer_, goog.Timer.TICK, this.onTick_)), this.ac_ && this.ac_.dismissOnDelay()
  }
};
goog.ui.AutoComplete.InputHandler.prototype.onTick_ = function() {
  this.update()
};
goog.ui.AutoComplete.InputHandler.prototype.onKeyDownOnInactiveElement_ = function(a) {
  this.handleFocus(a)
};
goog.ui.AutoComplete.InputHandler.prototype.onKey_ = function(a) {
...
goog.ui.AutoComplete.InputHandler.prototype.onKeyUp_ = function(a) {
...
goog.ui.AutoComplete.InputHandler.prototype.startWaitingForIme_ = function() {
  if(!this.waitingForIme_) {
    this.eh_.listen(this.activeElement_, goog.events.EventType.KEYUP, this.onKeyUp_), this.eh_.listen(this.activeElement_, goog.events.EventType.KEYPRESS, this.onKeyPress_), this.waitingForIme_ = true
  }
};
goog.ui.AutoComplete.InputHandler.prototype.stopWaitingForIme_ = function() {
  if(this.waitingForIme_) {
    this.waitingForIme_ = false, this.eh_.unlisten(this.activeElement_, goog.events.EventType.KEYPRESS, this.onKeyPress_), this.eh_.unlisten(this.activeElement_, goog.events.EventType.KEYUP, this.onKeyUp_)
  }
};
goog.ui.AutoComplete.InputHandler.prototype.onIeKeyPress_ = function(a) {
  this.handleSeparator_(a)
};
goog.ui.AutoComplete.InputHandler.prototype.update = function(a) {
  if(a || this.activeElement_ && this.getValue() != this.lastValue_) {
    if(a || !this.rowJustSelected_) {
      a = this.parseToken(), this.ac_ && (this.ac_.setTarget(this.activeElement_), this.ac_.setToken(a, this.getValue()))
    }
...
goog.ui.AutoComplete.InputHandler.prototype.trim_ = function(a) {
  return this.trimmer_ ? String(a).replace(this.trimmer_, "") : a
};
goog.ui.AutoComplete.InputHandler.prototype.getTokenIndex_ = function(a, b) {
...
goog.ui.AutoComplete.InputHandler.prototype.splitInput_ = function(a) {
  if(!this.multi_) {
    return[a]
  }
  for(var a = String(a).split(""), b = [], c = [], d = 0, e = false;d < a.length;d++) {
    if(this.literals_ && this.literals_.indexOf(a[d]) != -1) {
      if(this.generateNewTokenOnLiteral_ && !e) {
        b.push(c.join("")), c.length = 0
      }
      c.push(a[d]);
      e = !e
    }else {
      !e && this.separators_.indexOf(a[d]) != -1 ? (c.push(a[d]), b.push(c.join("")), c.length = 0) : c.push(a[d])
    }
  }
  b.push(c.join(""));
  return b
...
goog.ui.AutoComplete.RemoteArrayMatcher = function(a, b) {
  goog.Disposable.call(this);
  this.url_ = a;
  this.useSimilar_ = !b;
...
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.content_ = void 0;
...
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.setMethod = function(a) {
  this.method_ = a
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.setContent = function(a) {
  this.content_ = a
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.setHeaders = function(a) {
  this.headers_ = a
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.setTimeoutInterval = function(a) {
  this.xhr_.setTimeoutInterval(a)
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.buildUrl = function(a, b, c, d) {
  a = new goog.Uri(a);
  a.setParameterValue("token", b);
  a.setParameterValue("max_matches", String(c));
  a.setParameterValue("use_similar", String(Number(d)));
  return a.toString()
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.shouldRequestMatches = function() {
  return true
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.parseResponseText = function(a) {
  var b = [];
  if(a) {
    try {
      b = goog.json.unsafeParse(a)
    }catch(c) {
    }
  }
  return b
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.xhrCallback = function(a, b, c) {
  c = c.target.getResponseText();
  b(a, this.parseResponseText(c))
};
goog.ui.AutoComplete.RemoteArrayMatcher.prototype.requestMatchingRows = function(a, b, c, d) {
  if(this.shouldRequestMatches(this.url_, a, b, this.useSimilar_, d) && (b = this.buildUrl(this.url_, a, b, this.useSimilar_, d))) {
    a = goog.bind(this.xhrCallback, this, a, c), this.xhr_.isActive() && this.xhr_.abort(), this.lastListenerKey_ && goog.events.unlistenByKey(this.lastListenerKey_), this.lastListenerKey_ = goog.events.listenOnce(this.xhr_, goog.net.EventType.SUCCESS, a), this.xhr_.send(b, this.method_, this.content_, this.headers_)
  }
};
...
goog.ui.AutoComplete.Renderer = function(a, b, c, d) {
  ...
  this.useStandardHighlighting_ = d != null ? d : true;
  this.highlightAllTokens_ = false;
  this.rightAlign_ = c != null ? c : false;
...
goog.ui.AutoComplete.Renderer.prototype.setTopAlign = function(a) {
  this.topAlign_ = a
};
goog.ui.AutoComplete.Renderer.prototype.setUseStandardHighlighting = function(a) {
  this.useStandardHighlighting_ = a
};
goog.ui.AutoComplete.Renderer.prototype.setHighlightAllTokens = function(a) {
  this.highlightAllTokens_ = a
};
goog.ui.AutoComplete.Renderer.prototype.renderRows = function(a, b, c) {
  this.token_ = b;
  this.rows_ = a;
  this.hilitedRow_ = -1;
...
goog.ui.AutoComplete.Renderer.prototype.dismiss = function() {
  ...
  if(this.visible_) {
    this.visible_ = false, goog.style.showElement(this.element_, false)
  }
};
goog.ui.AutoComplete.Renderer.prototype.show = function() {
  if(!this.visible_) {
    this.visible_ = true, goog.style.showElement(this.element_, true)
  }
};
goog.ui.AutoComplete.Renderer.prototype.isVisible = function() {
  return this.visible_
};
goog.ui.AutoComplete.Renderer.prototype.hiliteRow = function(a) {
  this.hiliteNone();
  this.hilitedRow_ = a;
  a >= 0 && a < this.element_.childNodes.length && (a = this.rowDivs_[a], goog.dom.classes.add(a, this.activeClassName, this.legacyActiveClassName_), this.target_ && goog.dom.a11y.setActiveDescendant(this.target_, a), goog.style.scrollIntoContainerView(a, this.element_))
};
...
goog.ui.AutoComplete.Renderer.prototype.hiliteId = function(a) {
  if(a == -1) {
    this.hiliteRow(-1)
  }else {
...
goog.ui.AutoComplete.Renderer.prototype.setMenuClasses_ = function(a) {
...
goog.ui.AutoComplete.Renderer.prototype.maybeCreateElement_ = function() {
  if(!this.element_) {
    ...
    goog.dom.a11y.setRole(a, goog.dom.a11y.Role.LISTBOX);
    a.id = goog.ui.IdGenerator.getInstance().getNextUniqueId();
    this.target_ && (goog.dom.a11y.setRole(this.target_, goog.dom.a11y.Role.COMBOBOX), goog.dom.a11y.setState(this.target_, goog.dom.a11y.State.AUTOCOMPLETE, "list"), goog.dom.a11y.setState(this.target_, goog.dom.a11y.State.HASPOPUP, true));
    this.dom_.appendChild(this.parent_, a);
    goog.events.listen(a, goog.events.EventType.CLICK, this.handleClick_, false, this);
    goog.events.listen(a, goog.events.EventType.MOUSEDOWN, this.handleMouseDown_, false, this);
    goog.events.listen(this.dom_.getDocument(), goog.events.EventType.MOUSEDOWN, this.handleDocumentMousedown_, false, this);
    goog.events.listen(a, goog.events.EventType.MOUSEOVER, this.handleMouseOver_, false, this)
  }
};
goog.ui.AutoComplete.Renderer.prototype.redraw = function() {
  ...
  if(this.customRenderer_ && this.customRenderer_.render) {
    ...
    var a = null;
    goog.iter.forEach(this.rows_, function(b) {
      b = this.renderRowHtml(b, this.token_);
      this.topAlign_ ? this.element_.insertBefore(b, a) : this.dom_.appendChild(this.element_, b);
      a = b
    }, this)
  }
  this.rows_.length == 0 ? this.dismiss() : (this.show(), this.preventMacScrollbarResurface_(this.element_), this.reposition(), goog.style.setUnselectable(this.element_, true))
};
goog.ui.AutoComplete.Renderer.prototype.reposition = function() {
  if(this.target_ && this.reposition_) {
    var a = goog.style.getPageOffset(this.target_), b = goog.style.getSize(this.target_), c = goog.style.getSize(goog.style.getClientViewportElement(this.target_)), d = goog.style.getSize(this.element_);
    a.y = this.topAlign_ ? a.y - d.height : a.y + b.height;
    (this.rightAlign_ || a.x + d.width > c.width) && this.keepAligned_ != "LEFT" ? (a.x = a.x + b.width - d.width, this.keepAligned_ = "RIGHT") : this.keepAligned_ = "LEFT";
    goog.style.setPageOffset(this.element_, a);
    if(this.topAlign_) {
      this.element_.style.visibility = "visible"
    }
  }
};
goog.ui.AutoComplete.Renderer.prototype.setAutoPosition = function(a) {
  this.reposition_ = a
};
goog.ui.AutoComplete.Renderer.prototype.disposeInternal = function() {
  ...
  if(this.element_) {
    goog.events.unlisten(this.element_, goog.events.EventType.CLICK, this.handleClick_, false, this), goog.events.unlisten(this.element_, goog.events.EventType.MOUSEDOWN, this.handleMouseDown_, false, this), goog.events.unlisten(this.dom_.getDocument(), goog.events.EventType.MOUSEDOWN, this.handleDocumentMousedown_, false, this), goog.events.unlisten(this.element_, goog.events.EventType.MOUSEOVER, this.handleMouseOver_, false, this), this.dom_.removeNode(this.element_), this.element_ = null, this.visible_ = 
    false
  }
  delete this.parent_
};
goog.ui.AutoComplete.Renderer.prototype.preventMacScrollbarResurface_ = function(a) {
...
goog.ui.AutoComplete.Renderer.prototype.renderRowContents_ = function(a, b, c) {
  c.innerHTML = goog.string.htmlEscape(a.data.toString())
};
goog.ui.AutoComplete.Renderer.prototype.hiliteMatchingText_ = function(a, b) {
  if(a.nodeType == goog.dom.NodeType.TEXT) {
    var c = null;
    goog.isArray(b) && b.length > 1 && !this.highlightAllTokens_ && (c = goog.array.slice(b, 1));
    var d = this.getTokenRegExp_(b);
    if(d.length != 0) {
      for(var e = a.nodeValue, f = RegExp("(.*?)(^|\\W+)(" + d + ")", "gi"), d = [], g = 0, h = f.exec(e), i = 0;h;) {
        i++, d.push(h[1]), d.push(h[2]), d.push(h[3]), g = f.lastIndex, h = f.exec(e)
      }
      d.push(e.substring(g));
      if(d.length > 1) {
        c = !this.highlightAllTokens_ ? 1 : i;
        for(e = 0;e < c;e++) {
          f = 3 * e, a.nodeValue = d[f] + d[f + 1], g = this.dom_.createElement("b"), g.className = this.highlightedClassName, this.dom_.appendChild(g, this.dom_.createTextNode(d[f + 2])), g = a.parentNode.insertBefore(g, a.nextSibling), a.parentNode.insertBefore(this.dom_.createTextNode(""), g.nextSibling), a = g.nextSibling
        }
        d = goog.array.slice(d, c * 3);
        a.nodeValue = d.join("")
      }else {
        c && this.hiliteMatchingText_(a, c)
      }
    }
  }else {
    for(d = a.firstChild;d;) {
      c = d.nextSibling, this.hiliteMatchingText_(d, b), d = c
    }
  }
};
goog.ui.AutoComplete.Renderer.prototype.getTokenRegExp_ = function(a) {
  var b = "";
  if(!a) {
    return b
  }
  this.highlightAllTokens_ ? goog.isArray(a) ? (a = goog.array.filter(a, function(a) {
    return!goog.string.isEmptySafe(a)
  }), a = goog.array.map(a, goog.string.regExpEscape), b = a.join("|")) : (b = goog.string.collapseWhitespace(a), b = goog.string.regExpEscape(b), b = b.replace(/ /g, "|")) : b = goog.isArray(a) ? a.length > 0 ? goog.string.regExpEscape(a[0]) : "" : goog.string.regExpEscape(a);
  return b
};
goog.ui.AutoComplete.Renderer.prototype.renderRowHtml = function(a, b) {
  var c = this.dom_.createDom("div", {className:this.rowClassName, id:goog.ui.IdGenerator.getInstance().getNextUniqueId()});
  goog.dom.a11y.setRole(c, goog.dom.a11y.Role.OPTION);
...
goog.ui.AutoComplete.Renderer.prototype.getRowFromEventTarget_ = function(a) {
  for(;a && a != this.element_ && !goog.dom.classes.has(a, this.rowClassName);) {
    a = a.parentNode
  }
  return a ? goog.array.indexOf(this.rowDivs_, a) : -1
};
goog.ui.AutoComplete.Renderer.prototype.handleClick_ = function(a) {
  var b = this.getRowFromEventTarget_(a.target);
  b >= 0 && this.dispatchEvent({type:goog.ui.AutoComplete.EventType.SELECT, row:this.rows_[b].id});
  a.stopPropagation()
};
goog.ui.AutoComplete.Renderer.prototype.handleMouseDown_ = function(a) {
...
goog.ui.AutoComplete.Renderer.prototype.handleMouseOver_ = function(a) {
  a = this.getRowFromEventTarget_(a.target);
  a >= 0 && !(goog.now() - this.startRenderingRows_ < goog.ui.AutoComplete.Renderer.DELAY_BEFORE_MOUSEOVER) && this.dispatchEvent({type:goog.ui.AutoComplete.EventType.HILITE, row:this.rows_[a].id})
};
...
goog.ui.AutoComplete.Remote = function(a, b, c, d) {
  this.matcher_ = a = new goog.ui.AutoComplete.RemoteArrayMatcher(a, !d);
  d = new goog.ui.AutoComplete.Renderer;
  c = new goog.ui.AutoComplete.InputHandler(null, null, !!c, 300);
  goog.ui.AutoComplete.call(this, a, d, c);
  c.attachAutoComplete(this);
  c.attachInputs(b)
};
goog.inherits(goog.ui.AutoComplete.Remote, goog.ui.AutoComplete);
goog.ui.AutoComplete.Remote.prototype.setUseStandardHighlighting = function(a) {
  this.renderer_.setUseStandardHighlighting(a)
};
goog.ui.AutoComplete.Remote.prototype.getInputHandler = function() {
  return this.selectionHandler_
};
goog.ui.AutoComplete.Remote.prototype.setMethod = function(a) {
  this.matcher_.setMethod(a)
};
goog.ui.AutoComplete.Remote.prototype.setContent = function(a) {
  this.matcher_.setContent(a)
};
goog.ui.AutoComplete.Remote.prototype.setHeaders = function(a) {
  this.matcher_.setHeaders(a)
};
goog.ui.AutoComplete.Remote.prototype.setTimeoutInterval = function(a) {
  this.matcher_.setTimeoutInterval(a)
};
var cljs = {core:{}};
