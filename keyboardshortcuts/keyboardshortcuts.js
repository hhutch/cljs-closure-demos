...
goog.ui.KeyboardShortcutHandler = function(a) {
  ...
  this.allShortcutsAreGlobal_ = this.alwaysStopPropagation_ = false;
...
goog.ui.KeyboardShortcutHandler.getKeyCode = function(a) {
  if(!goog.ui.KeyboardShortcutHandler.nameToKeyCodeCache_) {
    var b = {}, c;
    for(c in goog.events.KeyNames) {
      b[goog.events.KeyNames[c]] = c
    }
...
goog.ui.KeyboardShortcutHandler.prototype.interpretStrokes_ = function(a, b) {
  var c;
  if(goog.isString(b[a])) {
    c = goog.ui.KeyboardShortcutHandler.parseStringShortcut(b[a])
  }else {
    var d = b, e = a;
    goog.isArray(b[a]) && (d = b[a], e = 0);
    for(c = [];e < d.length;e += 2) {
      c.push({keyCode:d[e], modifiers:d[e + 1]})
    }
  }
  return c
};
...
goog.ui.KeyboardShortcutHandler.parseStringShortcut = function(a) {
  for(var a = a.replace(/[ +]*\+[ +]*/g, "+").replace(/[ ]+/g, " ").toLowerCase(), a = a.split(" "), b = [], c, d = 0;c = a[d];d++) {
    c = c.split("+");
    for(var e, f = goog.ui.KeyboardShortcutHandler.Modifiers.NONE, g, h = 0;g = c[h];h++) {
      switch(g) {
        case "shift":
          f |= goog.ui.KeyboardShortcutHandler.Modifiers.SHIFT;
          continue;
        case "ctrl":
          f |= goog.ui.KeyboardShortcutHandler.Modifiers.CTRL;
          continue;
        case "alt":
          f |= goog.ui.KeyboardShortcutHandler.Modifiers.ALT;
          continue;
        case "meta":
          f |= goog.ui.KeyboardShortcutHandler.Modifiers.META;
          continue
      }
      e = goog.ui.KeyboardShortcutHandler.getKeyCode(g);
      break
    }
    b.push({keyCode:e, modifiers:f})
  }
  return b
};
goog.ui.KeyboardShortcutHandler.prototype.initializeKeyListener = function(a) {
  ...
  goog.userAgent.MAC && goog.userAgent.GECKO && goog.userAgent.isVersion("1.8") && goog.events.listen(this.keyTarget_, goog.events.EventType.KEYUP, this.handleMacGeckoKeyUp_, false, this);
  goog.userAgent.WINDOWS && !goog.userAgent.GECKO && (goog.events.listen(this.keyTarget_, goog.events.EventType.KEYPRESS, this.handleWindowsKeyPress_, false, this), goog.events.listen(this.keyTarget_, goog.events.EventType.KEYUP, this.handleWindowsKeyUp_, false, this))
};
goog.ui.KeyboardShortcutHandler.prototype.handleMacGeckoKeyUp_ = function(a) {
  if(a.keyCode == goog.events.KeyCodes.MAC_FF_META) {
    this.metaKeyRecentlyReleased_ = true, goog.Timer.callOnce(function() {
      this.metaKeyRecentlyReleased_ = false
    }, 400, this)
  }else {
    var b = a.metaKey || this.metaKeyRecentlyReleased_;
    if((a.keyCode == goog.events.KeyCodes.C || a.keyCode == goog.events.KeyCodes.X || a.keyCode == goog.events.KeyCodes.V) && b) {
      a.metaKey = b, this.handleKeyDown_(a)
    }
  }
};
...
goog.ui.KeyboardShortcutHandler.prototype.handleWindowsKeyPress_ = function(a) {
  if(a.keyCode > 32 && this.isPossiblePrintableKey_(a)) {
...
goog.ui.KeyboardShortcutHandler.prototype.clearKeyListener = function() {
  ...
  goog.userAgent.MAC && goog.userAgent.GECKO && goog.userAgent.isVersion("1.8") && goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYUP, this.handleMacGeckoKeyUp_, false, this);
  goog.userAgent.WINDOWS && !goog.userAgent.GECKO && (goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYPRESS, this.handleWindowsKeyPress_, false, this), goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYUP, this.handleWindowsKeyUp_, false, this));
  this.keyTarget_ = null
};
goog.ui.KeyboardShortcutHandler.setShortcut_ = function(a, b, c) {
  var d = b.shift(), d = goog.ui.KeyboardShortcutHandler.makeKey_(d.keyCode, d.modifiers), e = a[d];
  if(e && c && (b.length == 0 || goog.isString(e))) {
    throw Error("Keyboard shortcut conflicts with existing shortcut");
  }
  b.length ? (e || (e = a[d] = {}), goog.ui.KeyboardShortcutHandler.setShortcut_(e, b, c)) : a[d] = c
};
goog.ui.KeyboardShortcutHandler.prototype.getShortcut_ = function(a, b, c) {
  b = b || 0;
  return(c = (c || this.shortcuts_)[a[b]]) && !goog.isString(c) && a.length - b > 1 ? this.getShortcut_(a, b + 1, c) : c
};
goog.ui.KeyboardShortcutHandler.prototype.checkShortcut_ = function(a) {
  for(var b = this.shortcuts_;a.length > 0 && b;) {
...
goog.ui.KeyboardShortcutHandler.prototype.handleKeyDown_ = function(a) {
  if(this.isValidShortcut_(a)) {
    if(a.type == "keydown" && this.isPossiblePrintableKey_(a)) {
      this.isPrintableKey_ = false
    }else {
      var b = goog.ui.KeyboardShortcutHandler.makeKey_(a.keyCode, (a.shiftKey ? goog.ui.KeyboardShortcutHandler.Modifiers.SHIFT : 0) | (a.ctrlKey ? goog.ui.KeyboardShortcutHandler.Modifiers.CTRL : 0) | (a.altKey ? goog.ui.KeyboardShortcutHandler.Modifiers.ALT : 0) | (a.metaKey ? goog.ui.KeyboardShortcutHandler.Modifiers.META : 0)), c, d, e = goog.now();
      this.lastKeys_.strokes.length && e - this.lastKeys_.time <= goog.ui.KeyboardShortcutHandler.MAX_KEY_SEQUENCE_DELAY ? c = this.getShortcut_(this.lastKeys_.strokes) : this.lastKeys_.strokes.length = 0;
      c = c ? c[b] : this.shortcuts_[b];
      if(!c) {
        c = this.shortcuts_[b], this.lastKeys_.strokes = []
      }
      c && goog.isString(c) ? d = c : c ? (this.lastKeys_.strokes.push(b), this.lastKeys_.time = e, goog.userAgent.GECKO && a.preventDefault()) : this.lastKeys_.strokes.length = 0;
      if(d) {
        this.alwaysPreventDefault_ && a.preventDefault(), this.alwaysStopPropagation_ && a.stopPropagation(), c = goog.ui.KeyboardShortcutHandler.EventType, e = a.target, b = this.dispatchEvent(new goog.ui.KeyboardShortcutEvent(c.SHORTCUT_TRIGGERED, d, e)), d = new goog.ui.KeyboardShortcutEvent(c.SHORTCUT_PREFIX + d, d, e), b &= this.dispatchEvent(d), b || a.preventDefault(), this.lastKeys_.strokes.length = 0
      }
    }
  }
};
goog.ui.KeyboardShortcutHandler.prototype.isValidShortcut_ = function(a) {
  var b = a.keyCode;
  if(b == goog.events.KeyCodes.SHIFT || b == goog.events.KeyCodes.CTRL || b == goog.events.KeyCodes.ALT) {
    return false
  }
  var c = a.target, d = c.tagName == "TEXTAREA" || c.tagName == "INPUT" || c.tagName == "BUTTON" || c.tagName == "SELECT", e = !d && (c.isContentEditable || c.ownerDocument && c.ownerDocument.designMode == "on");
  return!d && !e ? true : this.globalKeys_[b] || this.allShortcutsAreGlobal_ ? true : e ? false : this.modifierShortcutsAreGlobal_ && (a.altKey || a.ctrlKey || a.metaKey) ? true : c.tagName == "INPUT" && (c.type == "text" || c.type == "password") ? b == goog.events.KeyCodes.ENTER : c.tagName == "INPUT" || c.tagName == "BUTTON" ? b != goog.events.KeyCodes.SPACE : false
};
...
var keyboardshortcuts = {setup:function() {
  var a = new goog.ui.KeyboardShortcutHandler(document), b = goog.ui.KeyboardShortcutHandler.Modifiers.NONE, c = goog.ui.KeyboardShortcutHandler.Modifiers.CTRL, d = goog.ui.KeyboardShortcutHandler.Modifiers.SHIFT, e = goog.ui.KeyboardShortcutHandler.Modifiers.ALT;
  a.registerShortcut("A", "a");
  a.registerShortcut("T E S T", "t e s t");
  a.registerShortcut("SHIFT_F12", "shift+f12");
  a.registerShortcut("SHIFT_F11 C", "shift+f11 c");
  a.registerShortcut("META_Y", "meta+y");
  a.registerShortcut("G S", "g s");
  a.registerShortcut("S", "s");
  a.registerShortcut("GOOG", goog.events.KeyCodes.G, b, goog.events.KeyCodes.O, b, goog.events.KeyCodes.O, b, goog.events.KeyCodes.G);
  a.registerShortcut("CTRL_A", goog.events.KeyCodes.A, c);
  a.registerShortcut("BC", goog.events.KeyCodes.B, b, goog.events.KeyCodes.C);
  a.registerShortcut("BD", goog.events.KeyCodes.B, b, goog.events.KeyCodes.D);
  a.registerShortcut("ALT_Q A", goog.events.KeyCodes.Q, e, goog.events.KeyCodes.A);
  a.registerShortcut("ALT_Q SHIFT_A", goog.events.KeyCodes.Q, e, goog.events.KeyCodes.A, d);
  a.registerShortcut("ALT_Q SHIFT_B", goog.events.KeyCodes.Q, e, goog.events.KeyCodes.B, d);
  a.registerShortcut("SPACE", goog.events.KeyCodes.SPACE);
  a.registerShortcut("HOME", goog.events.KeyCodes.HOME);
  a.registerShortcut("ENTER", goog.events.KeyCodes.ENTER);
  return goog.events.listen(a, goog.ui.KeyboardShortcutHandler.EventType.SHORTCUT_TRIGGERED, function(a) {
    return new goog.dom.setTextContent(new goog.dom.getElement("text"), cljs.core.str.call(null, "Shortcut Triggered: ", a.identifier))
  })
}};
goog.exportSymbol("keyboardshortcuts.setup", keyboardshortcuts.setup);
