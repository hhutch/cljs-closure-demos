...
(defn ^:export setup []
  (doseq [btn [1 2 3 4 6 7 8]]
    (.listen goog.events (dom/get-element (str "button" btn))
                         ...
                         (fn [e] (click-button (str "button" btn)
                                               (str "textField" btn))))) 
  ...
  (.listen goog.events (dom/get-element "button5.0.1")
                        goog.events.EventType.CLICK
                       (fn [e] (js/alert 
                                (str "goog.ui.Button doesn't let me use an existing element?  "  
                                     "how do you keep track of an anonymous element in a non-mutable "
                                     "non-global context?")
                                ))))
